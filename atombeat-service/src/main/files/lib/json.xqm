(:~
 :    Tweaked version of the JSON module that comes with eXist, to use element local names instead 
 :    of tag names in object keys, so JSON clients don't get confused by inconsistent namespace
 :    prefixes.
 : 
 ...
 :)
module namespace json = "http://purl.org/atombeat/xquery/json" ;
...
declare function json:element-helper($attribs as attribute()*, $children as node()*) {
    string-join((
        ...
        if ($children instance of text()+) then
            ...
            for $name in distinct-values(for $c in $children return local-name($c))
            return
                json:node-to-json($children[local-name(.) = $name])
    ),
    ', ')
};
...
declare function json:node-to-json($node as node()+) {
    typeswitch ($node)
        case $elem as element() return
            concat('"', local-name($elem), '" : ', json:contents-to-json($node))
        ...
        case $attr as attribute() return
            concat('"@', local-name($attr), '": ', json:contents-to-json($attr))
        default return
