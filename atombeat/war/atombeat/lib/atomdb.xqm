...
(:~
 ...
 : @return the eXist database path where the entry is stored, or empty if no member is available at that location.
 :)
...
) as xs:string?
{
	...
	then ()
		...
		let $entry-db-path := atomdb:store-member( $collection-path-info , $entry-resource-name , $new )
